...
if __name__ == "__main__":
    ...
    #Taille du labyrinthe prise en argument si l'on veut :
    size_laby = 25, 25
    if len(sys.argv) > 2:
        size_laby = int(sys.argv[1]),int(sys.argv[2])

    #On ouvre "pygame" et on initialise l'écran
    pg.init()
    resolution = size_laby[1]*8, size_laby[0]*8
    screen = pg.display.set_mode(resolution)

    #Le nombre de fourmis dépend de la taille du labyrinthe :
    nb_ants = size_laby[0]*size_laby[1]//4//(nbp-1) if rank !=0 else size_laby[0]*size_laby[1]//4
    max_life = 500
    #Leur durée de vie peut être prise en argument si l'on veut :
    if len(sys.argv) > 3:
        max_life = int(sys.argv[3])

    #La nourriture est tout en bas à droite et le nid des fourmis en haut à gauche
    ...
    #La partie qui suit sert à la diffusion des phéromones :
    alpha = 0.9
    beta  = 0.99
    #On peut modifier alpha et bêta si l'on veut :
    if len(sys.argv) > 4:
    ...
    #Le processus 0 s'occupe de l'affichage, donc aussi de l'affichage du labyrinthe
    if rank == 0:
    ...
    while True:
        #permet de fermer pygame
        for event in pg.event.get():
            if event.type == pg.QUIT:
                pg.quit()
                exit(0)

        #Le processus zéro est le processus principal qui gère l'affichage :
        if rank == 0:
            deb = time.time()
            food_counter = 0
            pherom.pheromon = np.zeros((size_laby[0]+2, size_laby[1]+2), dtype=np.double)
            
            #Le processus zéro doit recevoir un certain nombre d'infos de la part des autres processus qui calculent
            for i in range(1, nbp):
                food_counter, position, directions = globCom.recv(source=MPI.ANY_SOURCE)
                food_counter += food_counter
                #On "réduit" le tableau, seule la position actuelle de la fourmi nous inntéresse
                ants.historic_path[nb_ants//(nbp-1)*(i-1):nb_ants//(nbp-1)*(i), np.zeros(nb_ants//(nbp-1), dtype=np.int_), :] = position 
                ants.directions[nb_ants//(nbp-1)*(i-1):nb_ants//(nbp-1)*(i)] = directions
        ...
        if rank != 0:
            ...
            # Envoie au processus zéro les informations utiles
            globCom.send((food_counter, ants.historic_path[:,ants.age,:], ants.directions), dest=0, tag=11)

            
        # pg.time.wait(500)
        
        # Partage les pheromones entre tous les processus
        phero_reduced = np.zeros(pherom.pheromon.shape, dtype=np.double)
        globCom.Allreduce([pherom.pheromon, MPI.DOUBLE], [phero_reduced, MPI.DOUBLE], op=MPI.MAX)
        pherom.pheromon = phero_reduced
        
        if rank == 0:
            #Le processus zéro s'occupe aussi d'afficher les phéromones.
